...
class BorutaShap:
    ...
    def Train_model(self, X, y, sample_weight = None):
        ...
        if 'catboost' in str(type(self.model)).lower():
        ...
        elif self.categorical_feature is not None:
            try:
                self.model.fit(X, y, sample_weight = sample_weight, categorical_feature=self.categorical_feature,
            ...
            try:
                self.model.fit(X, y, sample_weight = sample_weight, feature_name=list(X.columns))
    ...
    def fit(self, X, y, sample_weight = None, n_trials = 20, random_state=0, sample=False,
            train_or_test = 'test', normalize=True, verbose=True, stratify=None, categorical_feature=None):
    ...
    def binomial_H0_test(array, n, p, alternative):
        ...
        """
        return [binomtest(x.astype(int), n=int(n), p=p, alternative=alternative).pvalue for x in array]
